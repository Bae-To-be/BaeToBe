...
Future<List<String>> companySuggestions(ref, query) async {
  List<String> result = [];
  final error = ref.read(errorProvider.notifier);
  final client = ref.read(networkClientProvider);
  await error.safelyExecute(
      command: client.get(BackendRoutes.searchCompanies,
          queryParameters: {'query': query}),
      onSuccess: (response) {
        final suggestions =
            List<Map<String, dynamic>>.from(response.data['data']);
        result =
            suggestions.map((company) => company['name'] as String).toList();

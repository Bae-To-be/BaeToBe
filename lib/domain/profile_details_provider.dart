...
final profileDetailsProvider =
  ...
  ref.watch(matchesProvider.select((result) => result.value
      ?.firstWhereOrNull((match) => match.basicProfile.userId == id)));
  // A like can be removed using a notification, in that case this API will return no user.
  ref.watch(likesProvider(likeDirection.sent).select((result) => result.value
      ?.firstWhereOrNull((like) => like.basicProfile.userId == id)));
